...
def _plot_baseline_trajectory_and_cost(
    ...
    for result in results:
        ...
        reward_values = [row["episode_reward_mean"] for row in result.episode_rows]
        defender_values = [row["defender_reward"] for row in result.episode_rows]
        reward_mean = _rolling_mean(reward_values, smooth_window)
        reward_std = _rolling_std(reward_values, smooth_window)
        ...
        axes[0].plot(xs, reward_mean, linewidth=2.5, label=result.variant_name, color=color)
        axes[0].fill_between(
            xs,
        ...
        axes[1].plot(xs, defender_mean, linewidth=2.5, label=result.variant_name, color=color)
        axes[1].fill_between(
            xs,
    ...
    axes[0].set_title("不同算法平均回报收敛轨迹")
    axes[0].set_xlabel("训练轮次")
    axes[0].set_ylabel("平均回报")
    axes[0].grid(False)
    axes[0].legend(frameon=False)

    axes[1].set_title("不同算法防御方回合收益轨迹")
    axes[1].set_xlabel("训练轮次")
    axes[1].set_ylabel("防御方平均回合收益")
    axes[1].grid(False)
    axes[1].legend(frameon=False)

    labels = [result.variant_name for result in results]
    costs = [result.summary_row["training_time_s"] for result in results]
    bars = axes[2].bar(labels, costs, color=[palette.get(label, "#999999") for label in labels], width=0.5)
    axes[2].set_title("不同算法总训练开销对比")
